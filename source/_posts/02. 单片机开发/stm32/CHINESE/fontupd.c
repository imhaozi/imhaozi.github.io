...
//字库存放起始地址
#define FONTINFOADDR    1024*1024*2                     //Pandora STM32开发板是从2M地址以后开始存放字库
...
uint8_t updata_fontx(uint16_t x, uint16_t y, uint8_t size, uint8_t *fxpath, uint8_t fx)
{
    ...
    uint32_t offx = 0;
    uint8_t rval = 0;
    fftemp = (FIL *)mymalloc(SRAM1, sizeof(FIL));   //分配内存

    if(fftemp == NULL)rval = 1;

    tempbuf = mymalloc(SRAM1, 4096);                //分配4096个字节空间
    ...
    myfree(SRAM1, fftemp);  //释放内存
    myfree(SRAM1, tempbuf); //释放内存
    return res;
}
...
uint8_t update_font(uint16_t x, uint16_t y, uint8_t size, uint8_t *src)
{
    ...
    res = 0XFF;
    ftinfo.fontok = 0XFF;
    pname = mymalloc(SRAM1, 100);   //申请100字节内存
    buf = mymalloc(SRAM1, 4096);    //申请4K字节内存
    fftemp = (FIL *)mymalloc(SRAM1, sizeof(FIL));   //分配内存
