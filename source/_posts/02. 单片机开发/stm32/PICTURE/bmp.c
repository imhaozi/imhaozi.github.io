...
/**
 * @brief	标准的bmp解码,解码filename这个BMP文件,主要是速度比较慢
 *
 * @param   filename	包含路径的文件名  
 *
 * @return  u8			0,成功;其他,错误码
 */
...
/**
 * @brief	小尺寸的bmp解码,解码filename这个BMP文件
 *
 * @param   filename		包含路径的文件名
 * @param   x,y				在屏幕上的起始坐标
 * @param   width,height	显示区域大小
 * @param   acolor			附加的alphablend的颜色(这个仅对32位色bmp有效!!!)
 * @param   mode			模式(除了bit5,其他的均只对32位色bmp有效!!!)
 ...
 * @param   width,height	显示区域大小
 * @param   acolor			附加的alphablend的颜色(这个仅对32位色bmp有效!!!)
 * @param   mode			模式.0,仅仅创建新文件的方式编码;1,如果之前存在文件,则覆盖之前的文件.如果没有,则创建新的文件 
 *
 * @return  u8			0,成功;其他,错误码
 */
...
{				
	...
	if((x+width-1)>LCD_Width)return PIC_WINDOW_ERR;		//区域错误
	if((y+height-1)>LCD_Height)return PIC_WINDOW_ERR;	//区域错误 
 	 
#if BMP_USE_MALLOC == 1	//使用malloc	
	databuf=(u16*)pic_memalloc(2048);		//开辟至少bi4width大小的字节的内存区域 ,对240宽的屏,480个字节就够了.最大支持1024宽度的bmp编码
	if(databuf==NULL)return PIC_MEM_ERR;		//内存申请失败.
	f_bmp=(FIL *)pic_memalloc(sizeof(FIL));	//开辟FIL字节的内存区域 
	if(f_bmp==NULL)								//内存申请失败.
	{		 
