...
{  
	LCD_Fill_Buf(x,y,x+width-1,y+height-1,color);//直接填充	
}
//////////////////////////////////////////////////////////////////////////
/**
 * @brief	画图初始化,在画图之前,必须先调用此函数,指定画点/读点
 *
 * @param   void
 *
 * @return  void
...
//	pic_phy.read_point=LCD_ReadPoint;  		//读点函数实现,仅BMP需要
	pic_phy.draw_point=LCD_Draw_ColorPoint;	//画点函数实现
...
/**
 * @brief	智能画图,图片在开始和结束的坐标点范围内显示
 *
 * @param  	FileName		要显示的图片文件  BMP/JPG/JPEG
 * @param  	x,y				坐标
 * @param  	width,height	显示区域尺寸
 * @param  	fast			使能jpeg/jpg小图片(图片尺寸小于等于液晶分辨率)快速解码,0,不使能;1,使能.
 *
 * @return  u8				0,不需要显示.1,需要显示
 */
