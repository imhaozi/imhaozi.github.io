...
{ 
	...
	/*设置LCD显示区域*/
	u16 x_s,y_s,x_e,y_e;
	x_s = ((picinfo.Div_Fac*(rect->left))>>13) + picinfo.S_XOFF;
	y_s = ((picinfo.Div_Fac*(rect->top))>>13) + picinfo.S_YOFF;
	x_e = ((picinfo.Div_Fac*(rect->left+width-1))>>13)+picinfo.S_XOFF;
	y_e = ((picinfo.Div_Fac*(rect->top+height-1))>>13) + picinfo.S_YOFF;
	LCD_Address_Set( x_s, y_s , x_e, y_e);
   	...
   	for(i=0;i<height;i++)//y坐标
	{
		...
		for(j=0;j<width;j++)//x坐标
		{
			...
			LCD_Write_HalfWord(*pencolor);//向LCD的显示区域写数据，用来显示图片(这种方法比上面方法稍微快一点)
			pencolor++;
		}
	}
    return 0;    //返回0,使得解码工作继续执行 
