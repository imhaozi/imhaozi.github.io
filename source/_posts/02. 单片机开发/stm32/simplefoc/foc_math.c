...
// 
int16_t f_round(float num)
{
    return (num >= 0) ? (uint8_t)(num + 0.5f) : (uint8_t)(num - 0.5f);
}

// receive an angle in between 0 and 2PI.
// precision +-0.005
float f_sin(float num)
{
    if (num < _PI_2)
    {
        return 0.0001f * sine_array[f_round(126.6873f * num)];
    }
    else if (num < _PI)
    {
        return 0.0001f * sine_array[398 - f_round(126.6873f * num)];
    }
    else if (num < _3PI_2)
    {
        return -0.0001f * sine_array[-398 + f_round(126.6873f * num)];
    ...
    {
        return -0.0001f * sine_array[796 - f_round(126.6873f * num)];
    }
}
...
float f_constrain(float num, float low, float high)
{
    if (num < low)
        return low;
